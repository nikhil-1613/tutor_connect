...
export default function AdminAssignedPage() {
  ...
  const handleMessageParent = (phoneNumber: string, tutorName: string) => {
    const message = `Hello! This is EduMatch. Your assigned tutor is ${tutorName}. Please feel free to discuss further details.`;
    const whatsappUrl = `https://wa.me/${phoneNumber.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  };

  const handleMessageTutor = (phoneNumber: string, parentName: string) => {
    const message = `Hello! This is EduMatch. You have been assigned to ${parentName}'s family. Please connect with them to discuss details.`;
    const whatsappUrl = `https://wa.me/${phoneNumber.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  };
